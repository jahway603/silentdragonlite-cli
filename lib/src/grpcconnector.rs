...
async fn get_coinsupply_info(uri: &http::Uri) -> Result<Coinsupply, Box<dyn std::error::Error>> {
    let mut client = get_client(uri).await?;
...
pub fn get_coinsupply(uri: http::Uri,) -> Result<Coinsupply, String> {
    let mut rt = tokio::runtime::Runtime::new().map_err(|e| e.to_string())?;
   
    rt.block_on(get_coinsupply_info(&uri)).map_err( |e| e.to_string())
